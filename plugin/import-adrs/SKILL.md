...
6. **Offer to retire the source documents.** With the ADRs committed, the scanned docs are redundant — and git still holds every original, so deleting them loses nothing. Offer to remove the source documents (every row in the ledger). On decline, stop here.

   Do it in two commits, because the log must cite a hash that doesn't exist until the deletion lands:

   - Delete every source document the ledger lists and commit — message `removing legacy docs imported by honist-v:import-adrs`.
   - Write `docs/adr/_drafts/STALE-DOCS-DELETION-LOG.md`: a line naming that commit's short hash, then the deleted paths one per line — enough for a future reader to recover any original with `git show <hash>:<path>`. Commit it alone — message `add docs/adr/_drafts/STALE-DOCS-DELETION-LOG.md`.

   Completion criterion: two commits landed — the deletion, then the log that names its hash and lists every path the deletion removed.
