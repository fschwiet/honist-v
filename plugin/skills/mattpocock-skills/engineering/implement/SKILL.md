...
Orchestrate the implementation of the tickets given by the user. Have a subagent implement the next unblocked ticket one at a time until all tickets are implemented.

The subagent should use /tdd where possible and check their work with stages of the verification pipeline relevant to their work (including at least formatting, linting and testing as outlined in the project's context). Before returning the agent should update the ticket with `Status: ready-for-review` and commit their work .
